...
function poll(timeout) {
    setTimeout(function () {
    	//PJB: If we are monitoring the Salesforce session
	if(monitorSFDCSession)
	{
		...
		//PJB: Track if the connection was denied so we don't keep retrying
		...
		else
		{
            ...
            error: function(xhr, ajaxOptions, thrownError) {
                ...
                if (thrownError == 'timeout') {
                    requestConnection();	//PJB: If we timeout, we should request a new connection in case Workspace has been restarted, not continue polling
                }
                else {
                    poll(pollQueueTimeout);
                }
    ...
    jQuery.ajax({
        ...
        success: function(data) {
	    	...
	    	if (data.action == 'ConnectionAccepted') {
	    		//PJB: Will we try to track the SalesForce session?
	        ...
	        else if (data.action == 'ConnectionDenied'){ //PJB: Don't retry again if the session was denied
	            stopTimer();
	        ...
	        else{ //ConnectionDenied (PJB: Not sure of any cases where this still gets called)
